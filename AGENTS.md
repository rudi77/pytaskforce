...
- Execution failures can be raised as `TaskforceExecutionError` derivatives (`AgentExecutionError`, `LeanAgentExecutionError`) in `src/taskforce/core/domain/exceptions.py`, which carry structured context like `session_id`, `agent_id`/`lean_agent_id`, `tool_name`, and `error_code` for executor logging.
