...
- LeanAgent planning strategy selection lives under `agent.planning_strategy` and optional `agent.planning_strategy_params` in profile YAML; supported values are `native_react` (default) and `plan_and_execute`.
