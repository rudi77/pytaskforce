...
For example, use a powerful reasoning model for planning and reflection,
and a fast/cheap model for summarization.

Routing is configured in `llm_config.yaml` (alongside model aliases, not in
the profile YAML). The router wraps the LLM provider and intercepts each
call, selecting the appropriate model based on configurable rules. Planning
strategies emit phase hints (`planning`, `reasoning`, `acting`, `reflecting`,
`summarizing`) that the router matches against.
