...
class TestInMemoryGatewayConversationStore:
    @pytest.mark.asyncio
...
class TestGatewayConversationStore:
    @pytest.mark.asyncio
    async def test_session_id_round_trip(self, tmp_path) -> None:
        store = GatewayConversationStore(work_dir=str(tmp_path))
        await store.set_session_id("telegram", "conv-1", "sess-abc")
        result = await store.get_session_id("telegram", "conv-1")
        assert result == "sess-abc"
    ...
    async def test_history_survives_reload(self, tmp_path) -> None:
        ...
        reloaded = GatewayConversationStore(work_dir=str(tmp_path))
