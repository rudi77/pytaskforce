...
This module ships the framework-generic ReAct kernel plus a handful of
specialist prompt fragments that predate the ``.agent.md`` format. Agent-
specific prompts belong in their respective agent packages as ``.agent.md``
files; entries remaining here are scheduled to be migrated in follow-up PRs.

Layered architecture:
- LEAN_KERNEL_PROMPT: Streamlined kernel for Agent with native tool calling.
- GENERAL_AUTONOMOUS_KERNEL_PROMPT: Legacy full ReAct kernel (unused).
- CODING_SPECIALIST_PROMPT: Deprecated — to be moved to taskforce_coding_agent.
- RAG_SPECIALIST_PROMPT:   Deprecated — to be moved to taskforce_rag_agent.
- WIKI_SYSTEM_PROMPT:      Deprecated — to be moved to taskforce_rag_agent.

Butler's prompt (formerly ``BUTLER_SPECIALIST_PROMPT``) has been migrated to
