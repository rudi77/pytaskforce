...
class WebhookEventSource:
    ...
    async def handle_inbound(
        ...
        """``WebhookCapableEventSource`` entry point used by the generic
        ``POST /api/v1/events/{source_name}`` route.

        Delegates to :meth:`handle_webhook` with ``source_label=source_name``
        and forwards the headers as metadata. Subclasses (GitHubWebhookSource)
        override this to add provider-specific signature checks before
        constructing the event.
        """
        return await self.handle_webhook(
            source_label=self._source_name,
            payload=payload,
    ...
    def from_config(
        ...
        *,
        ...
        """Factory used by ``EventSourceRegistry`` (config keys are optional)."""
        return cls(
            event_callback=event_callback,
            source_name=config.get("source_name", "webhook"),
        )
