...
class CalendarEventSource(PollingEventSource):
    ...
    def from_config(
        ...
        *,
        ...
        Accepts the YAML schema documented in the class docstring; falls
        back to safe defaults so the daemon can construct a no-op source
        when credentials are not configured.
        """
        poll_minutes = config.get("poll_interval_minutes", 5)
        return cls(
            poll_interval_seconds=float(poll_minutes) * 60.0,
            lookahead_minutes=int(config.get("lookahead_minutes", 60)),
