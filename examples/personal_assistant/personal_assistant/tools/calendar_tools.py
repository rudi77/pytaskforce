...
def _load_credentials(auth: CalendarAuthConfig):
    ...
    if auth.access_token:
        creds = Credentials(token=auth.access_token, scopes=CALENDAR_SCOPES)
    elif auth.token_file:
        creds = Credentials.from_authorized_user_file(auth.token_file, CALENDAR_SCOPES)
    else:
        raise ValueError("Provide access_token or token_file for GoogleCalendarTool")
