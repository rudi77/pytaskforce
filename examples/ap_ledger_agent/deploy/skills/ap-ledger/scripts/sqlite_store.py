...
typed query helpers. All paths are resolved via environment variables
or relative to the skill directory.
"""
...
# Resolve asset paths relative to the skill directory (scripts/../assets/)
_SCRIPTS_DIR = Path(__file__).resolve().parent
_SKILL_DIR = _SCRIPTS_DIR.parent
_ASSETS_DIR = _SKILL_DIR / "assets"
_SCHEMA_PATH = _ASSETS_DIR / "schema.sql"

# Country-specific seed data files
_SEED_PATHS: dict[str, Path] = {
    "AT": _ASSETS_DIR / "seed-data-at.sql",
    "DE": _ASSETS_DIR / "seed-data-de.sql",
}
...
class SQLiteStore:
    ...
    def ensure_initialized(self) -> None:
        ...
        seed_path = _SEED_PATHS.get(self.country)
        if not seed_path or not seed_path.exists():
            logger.warning("No seed data found for country=%s", self.country)
            seed_path = None
    ...
    def resolve_vendor(self, name: str) -> list[Vendor]:
        ...
        try:
            ...
            all_vendors = conn.execute(
                "SELECT * FROM vendors WHERE match_keywords IS NOT NULL"
            ).fetchall()
            matches = []
        ...
        try:
            ...
            if status == "posted":
                ...
                result["hint"] = (
                    ...
                    "Use journal_persist + journal_post to create "
    ...
    def euer_summary(self, year: int) -> list[dict[str, Any]]:
        """Return EUER category breakdown for a year."""
        conn = self._connect()
        try:
            rows = conn.execute(
    ...
    def export_csv(self, year: int) -> str:
        ...
        try:
            ...
            writer.writerow([
                "datum", "lieferant", "kategorie", "netto", "ust", "brutto", "steuersatz"
            ])
            for r in rows:
                writer.writerow([
                    r["invoice_date"] or "",
    ...
    def get_connection(self) -> sqlite3.Connection:
        """Return a raw connection for custom queries."""
