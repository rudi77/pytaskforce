...
# Account mappings
CATEGORY_TO_ACCOUNT = {
    "einnahmen_bar": ("4000", "Erloese Bareinnahmen"),
    "einnahmen_karte": ("4010", "Erloese Karteneinnahmen"),
    "einnahmen_gutschein": ("4020", "Erloese Gutscheine"),
    "einnahmen_sonstige": ("4090", "Sonstige Erloese"),
    ...
    "geraete": ("6800", "Werkzeuge & Geraete"),
    "buero": ("6820", "Buerobedarf"),
    "kfz": ("6900", "KFZ-Kosten"),
    "bank": ("6950", "Bankgebuehren"),
...
def main() -> None:
    ...
    try:
        if not args.tax_code:
            ...
            standard = max(
                (c for c in codes if c["rate"] > 0), key=lambda c: c["rate"], default=None
            )
            if standard:
