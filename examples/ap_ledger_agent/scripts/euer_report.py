"""Generate EÜR reports, monthly summaries, and CSV exports.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate EÜR reports")
    parser.add_argument("--action", required=True, choices=["monthly", "euer", "csv", "open", "categories"])
    ...
    if args.action == "monthly":
        ...
        if not csv_content:
            output({"success": True, "csv": "", "message": f"Keine gebuchten Belege für {args.year}."})
        else:
            output({"success": True, "csv": csv_content, "message": f"CSV-Export für {args.year} erstellt."})
        ...
        output({
            "success": True,
            "categories": [
                {"code": c.code, "name": c.name, "type": c.type.value, "default_tax_code": c.default_tax_code}
                for c in categories
            ],
        })
