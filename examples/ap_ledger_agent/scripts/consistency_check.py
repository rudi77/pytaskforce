...
Detects:
- Orphaned invoices (validated/posted without journal entries)
- Unposted journals (draft status linked to posted invoices)
- Unbalanced journals (posted entries where Soll != Haben)

Usage:
  python consistency_check.py
  python consistency_check.py --fix-hint   # include fix suggestions
"""
...
def main() -> None:
    ...
    conn = store._connect()  # reuse store's connection setup (WAL, FK pragmas)

    issues = []

    # 1. Orphaned invoices: validated/posted but no journal entry
    rows = conn.execute(
    ...
    # 4. Failed bookings in audit log (informational)
    rows = conn.execute(
